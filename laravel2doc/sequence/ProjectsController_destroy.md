sequenceDiagram
    autonumber
    participant C as Client
    participant R as Route
    participant ProjectsController as ProjectsController
    participant Model as Model
    participant DB as Database
    
    C->>R: DELETE /resource/{id}
    R->>+ProjectsController: destroy(id)
    ProjectsController->>+Model: find(id)
    Model->>+DB: SELECT * FROM table WHERE id = ?
    DB-->>-Model: Return record
    Model-->>-ProjectsController: Model instance
    ProjectsController->>+Model: delete()
    Model->>+DB: DELETE FROM table WHERE id = ?
    DB-->>-Model: Success
    Model-->>-ProjectsController: Success
    ProjectsController-->>-R: Return JSON response
    R-->>C: 204 No Content
    
    Note over ProjectsController,Model: This sequence removes a resource
  