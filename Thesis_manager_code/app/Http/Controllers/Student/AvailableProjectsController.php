...
class AvailableProjectsController extends Controller
{
    ...
    public function index()
    {
        ...
        $projects = Project::query()
            ...
            ->when(Auth::user()->specialization_id, function ($query, $specialization_id) {
    ...
    public function show(Project $project)
    {
       abort_if($project->status !== 'Approved', 404);
        ...
        // Get available students for group requests (same specialization, not assigned yet)
        $students = User::role('student')
            ->where('specialization_id', $project->specialization_id)
    ...
    public function requestProject(Request $request, Project $project)
    {
        ...
        DB::beginTransaction();
        try {
            $isIndividual = $validated['request_type'] === 'individual';
            ...
            $projectRequest = ProjectRequest::create([
                ...
                'reviewed_at' => $isIndividual ? now() : null,
            ]);

            if ($isIndividual) {
                // Assign the requesting student
                ...
                // Notify the student
                Auth::user()->notify(new ProjectAssignedNotification($project));

                // Load relationships for notification to admins
                $projectRequest->load(['project', 'student']);

                // Notify all admins about the auto-approved project request
                ...
                return Redirect::route('student.assigned-projects.index')
                    ->with('success', 'Project request approved and assigned successfully!');
            }

            // For group requests, keep it pending and notify admins
            $projectRequest->load(['project', 'student']);
            $admins = User::role('admin')->get();
            foreach ($admins as $admin) {
                $admin->notify(new ProjectRequestNotification($projectRequest));
            ...
            return Redirect::route('student.available-projects.index')
                ->with('success', 'Project request submitted successfully and is awaiting admin approval.');
        } catch (\Exception $e) {
            DB::rollBack();
