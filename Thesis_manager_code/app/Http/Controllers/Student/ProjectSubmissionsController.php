...
class ProjectSubmissionsController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($selectedProject) {
            // Load specialization to get chapters
            $selectedProject->load('specialization');
        ...
        // Get chapter options from specialization or use default
        $chapterOptions = $selectedProject && $selectedProject->specialization 
            ? $selectedProject->specialization->chapters
            : [
                'Introduction',
                'Literature Review',
                'Methodology',
        ...
        return Inertia::render('Student/ProjectSubmissions/Index', [
            ...
            'chapterTemplate' => $selectedProject && $selectedProject->specialization 
                ? $selectedProject->specialization->chapter_template_path 
                : null,
        ]);
    }
    ...
    {
        // Get project to validate chapter_type against specialization chapters
        $project = Project::with('specialization')->findOrFail($request->project_id);
        $allowedChapters = $project->specialization 
            ? $project->specialization->chapters
            : [
                'Introduction',
                'Literature Review',
                'Methodology',
        ...
        $request->validate([
            ...
            'chapter_type' => ['required', 'string', function ($attribute, $value, $fail) use ($allowedChapters) {
                if (!in_array($value, $allowedChapters)) {
                    $fail('The selected chapter type is invalid for this specialization.');
                }
            }],
