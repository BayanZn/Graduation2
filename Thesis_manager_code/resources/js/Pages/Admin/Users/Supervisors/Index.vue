...
watch(
    params,
    debounce(() => {
        router.get(route('supervisors.index'), pickBy(params), {
            preserveScroll: true,
            preserveState: true,
        });
    }, 300),
...
const deleteSupervisor = (supervisor) => {
    ...
    AlertWithButtons.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('users.destroy', supervisor), {
                preserveScroll: true,
                onSuccess: () => {
                    Toast.fire({
                        icon: 'warning',
                    ...
                    <div class="row">
                        <div class="col-md-8 mb-3 mb-md-0">
                            <Link :href="route('users.create')" class="btn btn-primary">
                                Add Supervisor <i class="ri-add-line"></i>
                            </Link>
                        </div>
                    ...
                    <div class="table-responsive min-h-table">
                        <table class="table table-centered table-nowrap mb-0 rounded">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(supervisor, index) in supervisors.data" :key="supervisor.id">
                                    <td class="text-center align-middle">{{ currentRow(index, supervisors) }}</td>
                                    ...
                                    <td class="text-center align-middle">
                                        <div
                                            v-for="(role, index) in supervisor.roles"
                                            :key="index"
                                    ...
                                    <td class="text-center align-middle">
                                        <span v-if="supervisor.specialization" class="badge bg-info">
                                            {{ supervisor.specialization.name }}
                                        </span>
                                        <span v-else class="text-muted">N/A</span>
                                    </td>
                                    <td class="text-center align-middle">
                                        <Link :href="route('users.edit', supervisor)" class="btn btn-info btn-sm me-2">
                                            <i class="ri-pencil-fill"></i>
                                        </Link>
                                        <a
                                            @click.prevent="deleteSupervisor(supervisor)"
                                            href=""
                                            id="delete"
                        ...
                        <div class="col-md-6 offset-md-4 d-flex justify-content-md-end">
                            <nav>
                                <ul class="pagination pagination-sm">
                                    <template v-for="(link, index) in supervisors.links">
