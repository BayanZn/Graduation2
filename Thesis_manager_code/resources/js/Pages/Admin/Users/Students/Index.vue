...
watch(
    params,
    debounce(() => {
        router.get(route('students.index'), pickBy(params), {
            preserveScroll: true,
            preserveState: true,
        });
    }, 300),
...
const deleteStudent = (student) => {
    ...
    AlertWithButtons.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('users.destroy', student), {
                preserveScroll: true,
                onSuccess: () => {
                    Toast.fire({
                        icon: 'warning',
                    ...
                    <div class="row">
                        <div class="col-md-8 mb-3 mb-md-0">
                            <Link :href="route('users.create')" class="btn btn-primary">
                                Add Student <i class="ri-add-line"></i>
                            </Link>
                        </div>
                        ...
                        <table class="table table-centered table-nowrap mb-0 rounded">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(student, index) in students.data" :key="student.id">
                                    ...
                                    <td class="text-center align-middle">
                                        <div
                                            v-for="(role, index) in student.roles"
                                            :key="index"
                                    ...
                                    <td class="text-center align-middle">
                                        <span v-if="student.specialization" class="badge bg-info">
                                            {{ student.specialization.name }}
                                        </span>
                                        <span v-else class="text-muted">N/A</span>
                                    </td>
                                    <td class="text-center align-middle">
                                        <Link :href="route('users.edit', student)" class="btn btn-info btn-sm me-2">
